...
3. beforeunload事件：允许页面在退出之前执行一些数据清理操作，还可以询问用户是否要离开当前页面
...
### 代码执行
变量和函数声明在代码里的位置是不会改变的，而且是在`编译阶段`被 JavaScript 引擎放入`内存`中;  
![示例图](https://static001.geekbang.org/resource/image/64/1e/649c6e3b5509ffd40e13ce9c91b3d91e.png)

**编译阶段**  
经过编译阶段，生成两部分内容：`执行上下文`和`可执行代码`  
![示例图](https://static001.geekbang.org/resource/image/06/13/0655d18ec347a95dfbf843969a921a13.png)
...
### 调用栈
栈（stack）是一种`先进后出`，用来管理函数调用关系的一种数据结构。  
每次调用函数都会将函数的执行上下文压入栈中，函数执行完成会弹出栈。
```js
// 下面函数执行的调用栈
var a = 2
function add(b,c){
...
addAll(3,6)
```
<img src="https://static001.geekbang.org/resource/image/a5/1d/a5d7ec1f8f296412acc045835b85431d.png" width="300" height="200" />=>
...
<img src="https://static001.geekbang.org/resource/image/d0/7b/d0ac1d6e77735338fa97cc9a3f6c717b.png" width="300" height="200" />

::: danger 栈溢出（Stack Overflow）
调用栈是有大小的，当入栈的执行上下文超过一定数目，JavaScript 引擎就会报错，我们把这种错误叫做`栈溢出`。
:::

### 作用域（scope）
`作用域`是指在程序中定义变量的区域，该位置决定了变量的生命周期。通俗地理解，作用域就是**变量与函数的可访问范围**，即作用域控制着变量和函数的可见性和生命周期。  
目前js具有`全局作用域`，`函数作用域`以及`块级作用域`（es6新增）  
::: tip es6
**let**，**const**，**块级作用域** 来解决变量提升问题。  
> 如何解决变量提升？  
...
> 4. 在词法环境内部，维护了一个小型栈结构，栈底是函数最外层的变量，进入一个作用域块后，就会把该作用域块内部的变量压到栈顶；当作用域执行完成之后，该作用域的信息就会从栈顶弹出
![示例图](https://static001.geekbang.org/resource/image/06/08/06c06a756632acb12aa97b3be57bb908.png)
:::
...
***整理阶段段***：让所有存活的对象都向一端移动，然后直接清理掉端边界以外的内存。
...
![示例图](https://static001.geekbang.org/resource/image/46/57/46fba54f54b9bd43918308f9f1ae1357.png)
